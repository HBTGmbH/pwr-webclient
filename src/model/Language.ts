import {APILanguage} from './APIProfile';
import {NEW_ENTITY_PREFIX} from './PwrConstants';
/**
 * Single, immutable language. These languages are usually provided by the API and should not get changed.
 */
export class Language {
    public readonly id: string;
    public readonly name: string;
    public readonly isNew: boolean;

    private static CURRENT_ID: number = 0;

    private constructor(id: string, name: string, isNew: boolean) {
        this.id = id;
        this.name = name;
        this.isNew = isNew;
    }

    public static fromAPI(apiLanguage: APILanguage): Language {
        return new Language(String(apiLanguage.id), apiLanguage.name, false);
    }

    /**
     * Creates a language that can be considered as 'new'.
     * A new language is persisted as a new entity when sent to the API for an update operation. A new
     * language has an ID prefixed with {@link NEW_ENTITY_PREFIX} to create a seperate namespace.
     */
    public static createNew(name: string): Language {
        return new Language(NEW_ENTITY_PREFIX + String(Language.CURRENT_ID++),name, true);
    }
    public toAPI(): APILanguage {
        return {
            id: this.isNew ? null : Number.parseInt(this.id),
            name: this.name
        }
    }

}